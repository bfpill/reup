...
![LLC vs Sparsity Comparison](/images/memorization-is-fragile/label_developmental.png)

In the MNIST setting, we find that clustering aLLC traces gives interpretable groups both at a label level as well as a more fine-grained, 'visual similiarity' level.

Firstly, as you can see in the above plots, at high temperature or early We find a strong trend where inputs which appear visually similar cluster strongly. For example, within the cluster corresponding to the "1's" the inputs are locally organized by angle,
...
Again, we are normalizing the traces in order to account for differences in initial loss between digit classes.

At high temperatures, we see strong clustering by digit,
...
<img
  ...
  alt="LLC vs Sparsity Comparison"
  style={{ display: 'block', width: '150%', marginLeft: '-4.5vw', maxWidth: 'none', marginTop: '70px', marginBottom: '70px'}}
/>
...
To demonstrate how the aLLC can be operationalized for practical anomaly detection, we implemented a simple unsupervised detection pipeline using the Isolation Forest algorithm. Rather than using just the the aLLC scalar value, we leveraged the entire degradation trace to capture the distinctive pattern of how memorized content behaves under parameter perturbation. For each input, we collected the loss at each SGLD step, creating a high-dimensional "trace vector." These vectors were normalized using L2 normalization to focus on the pattern of degradation rather than absolute magnitude. The Isolation Forest then identified unusual patterns within these normalized traces.
