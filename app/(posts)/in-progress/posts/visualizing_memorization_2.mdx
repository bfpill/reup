---
title: "Tracing Circuits - A Complexity First Interpretability Method"
time:
...
Specifically, we train a two layer transformer to perform both modular addition and modular multiplication, where addition is done mod $p_1$ and multiplication is mod $p_2$. Multiplcation is just repeated addition, but a so in principle both circuits *could* overlap in how they process in numbers, but we believe this unlikely. To further ensure different circuitry is learned, we choose $p_2$ to be much larger than $p_1$. 

To mitigate any possibility that our aLLC traces are somehow getting interference by logits boosting (which we also believe is unlikely), we tokenize our task in a relatively contrived manner to ensure each input contains the same number and type of tokens: 
...
<img
  ...
  alt="LLC vs Sparsity Comparison"
  style={{ display: 'block', width: '250%', marginLeft: '-62%', maxWidth: 'none', marginTop: '70px', marginBottom: '70px'}}
/>
...
<div style={{ display: 'flex', alignItems: 'flex-start', marginTop: '30px', marginBottom: '30px' }}>
  <img
    ...
    alt="LLC vs Sparsity Comparison"
    style={{ width: '50%', marginRight: '20px', maxWidth: 'none' }}
  />
  Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. 
</div>
...
<div style={{ display: 'flex', alignItems: 'flex-start', marginTop: '0px', marginBottom: '30px' }}>
  <div style={{ marginRight: '20px' }}>
    This plot shows the model's loss for each input $a, b$ on the multiplication task. Once again, we see that the aLLC is not just reflecting the initial loss.
  </div>
  <img
...
That said, it should be possible to identify more than just circuits 'above' and 'below' the efficieny cutoff our temperature induces. By studying the behaviour of our clustering over a sweep of temperatures we should be able to learn about the full variety of 'circuits' our model implements, as in theory we expect most circuits to have at least slightly different efficiencies (How measurable these differences are by our current tools is another question)
...
We see that the losses on mislabeled points and normal points converge to a steady value - we find that traces which converge give more interpretable clusters than traces that don't, both at the memorization v generalization level and the by-label level.
...
* More techniques and experiments using aLLC traces for interpretability 
  * What is the best way to integrate data at various temperatures? 
