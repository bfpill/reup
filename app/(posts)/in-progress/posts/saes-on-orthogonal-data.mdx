...
time:
  created: "2024-19-13T09:56:06.854Z"
  updated: "2024-19-13T06:19:19.634Z"
---
...
Looking at the activations and decoder weight graphs for this model, we can see that the model is putting features into superposition—some decoder weights look to multiple neurons in the hidden layer to reconstruct the input feature. Interestingly, we can see in the above graphs that the model is putting features 3, 4, and 6 into superposition, and they form a triangle. When decoding a feature in superposition, the decoder fires strongly negative on one feature out of the three (one 'vertex' of the triangle)—this negative firing negates the positive firing along the shared dimension of a mismatch. On the other hand, when there's no superposition, the decoder weights will just be a scaled transpose of the hidden activations for each feature.
