...
Training a neural network is analogous to spreading a bucket of seeds over open an pasture and letting them grow untouched for a million years. Over time all variety of plant life will emerge, compete for sunlight,  
...
In the following sections we will use the aLLC will give us a scalar metric of 'complexity' which we can use to distinguish memorized inputs from inputs with generalizing features, as well as a vector of per-sample losses at each step of SGLD which we will use to visualize how different inputs degrade during the sampling process. 
...
The strong visual difference between high and low aLLC inputs suggests
...
<img
  ...
  alt="LLC vs Sparsity Comparison"
  style={{ display: 'block', width: '200%', marginLeft: '-34%', maxWidth: 'none', marginTop: '100px', marginBottom: '100px'}}
/>
