...
1. We take the core SLT technique, LLC estimation, and extend it to single inputs to accurately measure per-input-complexity where NTK and Hessian-based methods offer only approximations. This allows us to detect memorized examples in an MNIST setting and memorized trojans in a Large Language Model.
2. We introduce a new technique for studying model internals using shared patterns of degradation between inputs when sampling locally from the posterior with Stochastic Langevin Gradient Descent (SLGD). 
3. We use this technique to study the internal structure of a Transformer trained to do both modular addition and modular multiplication, where we find very interpretable structure. We then study the effects of temperature on this shared-circuitry-analysis and demonstrate how temperature acts to 'renormalize' observable model structure.
...
What are the defining differences in how memorized points and normal points behave as we traverse local parameter space? In the next section, we look at these patterns closer, and discuss why *temperature* is the driving force for differentiating circuits by their efficiency. 

# A Gradual Noising

The human eye is astonishingly good at picking out patterns from noise. In our case, the pattern *is* noise.
As we increase temperature, we uniformly noise out circuits below our efficient 
...
This plot shows how the loss degrades for both memorized and normal points over the course of sampling with SGLD. We see that both mislabeled (memorized) and normal inputs all have roughly equal, low losses before the first sampling step, but that after a couple of steps memorized points are high loss while normal points are less so. The squiggly lines above the heatmaps are [estimates](https://en.wikipedia.org/wiki/Kernel_density_estimation) of the distribution of losses at each step for both the memorized and normal points, which shows us that memorized points degrade along a much flatter distribution, where normal points sit around 0 with a slight fat right tail. 


The below plot shows tSNE clusterings of input traces and LLC estimates, collected at different temperatures, over varying stages of training. The points in each tSNE correspond to inputs, where red represents mislabeled inputs (from the train set) and blue normal inputs (from the test set, to ensure no *rote* memorization).

<img
  src="/images/memorization-is-fragile/developmental_and_temp_mnist.png"
  alt="LLC vs Sparsity Comparison"
  style={{ display: 'block', width: '200%', marginLeft: '-32%', maxWidth: 'none', marginTop: '100px', marginBottom: '100px'}}
/>

We see that mislabeled points form their own, distinct cluster as training progresses. We also see that high temperature LLC estimation for mislabeled points consistently *increases* over training, where as the 
