...
Neural Networks are learning machines not unlike gardens. When you train a model, you are growing a garden of *circuits*. Circuits can be simple or complex, useful or useless - all properties which inform the development and behaviour of our models.

Simple circuits are simple because they are made up of a small number of parts put together in a simple way, which generally manifests as their behaviour being robust to small perturbations in both individual parts and their interconnections. Complex circuits are complex because they require a precise combination of highly detailed parts, which generally manifests as their behaviour being fragile and breaking if any part changes even slightly. We can describe a models behaviour on an input as falling on a spectrum of complexity - some inputs demand complex circuits, for others simple mechanisms suffice.
...
To start on this question, we will operationalize existing measures of complexity coming out of [Singular Learning Theory](https://www.lesswrong.com/w/singular-learning-theory). Specifically, by tracking per-input losses during sampling from the *tempered* posterior, we firstly show that we can __differentiate 'complex' inputs from 'simple' inputs__, and secondly that when sampling from the tempered posterior different inputs have __distinct patterns of degradation which we can utilize for interpretability purposes__. We also explore the effects of temperature on the observed underlying structure, and argue that different temperatures reveal qualitatively different structure.

We first demonstrate this technique by detecting memorization in [MNIST](https://en.wikipedia.org/wiki/MNIST_database) models, then we move to interpreting internal structure in a two-layer Transformer trained to do both modular addition and modular multiplication. Finally, we show that our approach scales to detecting when a Large Language Model has memorized an output (a form of [Mechanistic Anomaly Detection](https://www.lesswrong.com/posts/vwt3wKXWaCvqZyF74/mechanistic-anomaly-detection-and-elk)).

Understanding circuits and their complexity is an essential prerequisite to understanding the behaviour of our modern and powerful models. For the purposes of this post, a circuit is any mechanism which effects a models *behaviour*. This definition frees us from caring about the particular inner workings of our models - instead of describing a model by the interactions of each and every little parameter we can describe it by it's function. 

# Prior Work

Early memorization detection in deep networks relied on influence functions to track training examples' effects on predictions, with [Koh and Liang (2017)](https://arxiv.org/abs/1703.04730) adapting these to deep models. These methods identified memorized examples through disproportionate influence but were constrained by local linear approximations and Hessian matrix complexity. NTK-based approaches ([Jacot et al., 2018](https://arxiv.org/abs/1806.07572)) provided theoretical insights into overparameterized models' ability to memorize but were largely restricted to infinite-width or linearized settings. Loss landscape analysis offered alternative perspectives, linking memorization to sharp directions in the [Hessian](https://en.wikipedia.org/wiki/Hessian_matrix) spectrum [[1]](https://paperswithcode.com/paper/understanding-memorization-in-generative), [[2]](https://arxiv.org/abs/2206.10654#:~:text=that%20,by%20scaling%20batch%20size%20and), while generalization correlated with [flat minima](https://www.researchgate.net/publication/14100213_Flat_Minima). Despite these insights, Hessian-based methods suffer from sensitivity to parameter transformations and limited expressiveness beyond local quadratic approximations. Alternative methods, such as [clustering per-sample gradients](https://arxiv.org/abs/2210.06759), attempt to identify memorized data through training dynamics but require careful timing and significant memory overhead.  
...
We take the core SLT technique, LLC estimation, and: 
...
Importantly are taking this average using a *temperature*, where $\beta^*$ represents the *inverse temperature*. Temperature controls the loss threshold below which our. which means that at low temperatures our loss is smooth and at high temperatures it is sharp. We will discuss temperature in more depth below, and investigate why it's
...
As discussed earlier, the LLC is calculated by sampling the average loss over a dataset of points at nearby, or 'local', parameters. If we modify our sampling process to collect *per-input* losses at each step of SGLD, we can learn about how each input contributes to the loss, from which we expect to be able to learn interesting things which an average loss couldn't tell us. For convenience, we will call this metric the Atomic Local Learning Coefficient (aLLC) *(because it sounds cool, there might not be any strong need to actually name this 'thing')*. 
...
We now setup an experiment where we are *sure* our model has memorized a set of points. To do so, we train a simple two layer, 256 hidden dimension ReLU classifier on digit images (the MNIST dataset), where 10% of the training examples are deliberately mislabeled. This creates two distinct populations within our training set:
...
*By training a simple linear regression on aLLC traces (introduced below), we can successfully classify mislabeled points with an overall accuracy of 0.99 across 6000 samples, where a precision of 0.96 for mislabeled examples indicates that some correctly labeled points display similar fragility patterns to memorized examples. Alternative methods that capture per-input complexity on MNIST-scale include [NTK](https://en.wikipedia.org/wiki/Neural_tangent_kernel) approximations, per-sample gradient metrics, and Hessian spectrum analyses. NTK methods, which treat wide networks as kernels, typically require computing an $$n\times n$$ matrix with [runtimes of $$O(n^2)$$–$$O(n^3)$$]((https://arxiv.org/abs/2111.11418)). Per-sample gradient techniques rank training examples by [the norm of their gradient or prediction error](https://arxiv.org/abs/2107.07075) and only add the cost of an extra backward pass per example, but don't do well in identifying *simple* examples. [Hessian-based](https://www.stat.berkeley.edu/~mmahoney/pubs/hessian_nips18.pdf#:~:text=a%20Hessian%20based%20study%20to,to%20favors%20flat%20areas%2C%20as) approaches, including influence functions, offer detailed curvature estimates but demand expensive $$O(P^2)$$ memory and $$O(P^3)$$ time computations. In [Superposition, Memorization, and Double Descent](https://transformer-circuits.pub/2023/toy-double-descent/index.html), Henighan et al. (2023) studied memorization in the superposition regime, showing how dataset size affects model representations. In a [footnote](https://arc.net/l/quote/wkzawsnc), Chris Olah introduced *fractional data dimensionality*, a metric quantifying how many independent feature directions a single data point occupies within a learned representation. Unlike curvature-based approaches, this method provides a direct view of representation complexity. However, its applicability remains limited, as it has only been demonstrated in toy settings where superposition properties can be explicitly measured in single-layer model. Collecting 1000-step aLLC traces for 3,000 inputs takes ~30 seconds on a standard macbook. To our knowledge, there doesn't appear to be a clear example of an easy to estimate per-input 'complexity' metric which captures a nuanced spectrum of complexity in the way we've demonstrated the aLLC can.*


# But Why? - *Temperature*

Why can we distinguish memorization from generalization? Let's start with a couple intuition bumps:
...
As briefly mentioned above, the temperature which we sample at during SGLD controls the 'tolerance' the sampler has points with high loss. Specifically, tuning the temperature changes the *sharpness* of the loss landscape which we are traversing. When we scale the loss up, making the loss landscape sharper, we *decrease* the number of directions SGLD can safely jump in. On the other hand, when we soften the loss, making the loss landscape flatter, we *increase* the number of directions. SGLD, which incorporates a random noise factor, is a semi-random walk around parameter space. By changing the sharpness of our loss landscape, we are able to control the probability of SGLD taking a step in directions which hurt the loss 'more than a little'. 
...
In practice, this means we can __control the 'importance' of circuits that SGLD is allowed to perturb__. When temperature is high, SGLD can only move in directions which barely hurt the loss - Importantly, directions which barely hurt the loss are *memorization* directions. 
...
We visualize the pattern of degradation for both memorized and normal points via heatmap. In order to properly visualize *patterns* of degradation and avoid observing simple differences in the magniture of the loss at each step, we normalize each trace with L2 normalization before clustering, which we find alters the shape of our tSNE clusters by centering the memorization cluster (displayed above each heatmap). 
...
* Visually similar inputs degrade in the same way because the model uses the same circuitry to identify them - the model 'perceives' them as being the same input. Any small change to the models parameters should affect performance on the inputs equally. 
* High aLLC points tend to be towards the center (where the mislabeled points are), indicating that the cluster is also organized by complexity. 


# Beyond Memorization - Finding More Complex Circuits

So far, we've explored how we can differentiate memorized inputs from inputs which rely on generalizing features by contrasting the effective 'complexity' of an input for a given model, which we called the aLLC. This was a baseline, and a sanity check - our end goal is being able to detect and interpret nuanced computational structure in our models, and memorization made a good test bed because it is simply the *least* efficient type of circuit and therefore should have the clearest differences with any *real* circuitry. 

A good setting for trying to distinguish between circuits is when we have a model trained to do a handful of completely different tasks. For extra technical jargon, we'd like our tasks to be orthogonal in function space, which means that getting better at task $A$ shouldn't mean you naturally get better at $B$. Ideally we'd like our model to have separate circuits for each task, but verifying this mis difficult and so to start with we’re going to work with tasks which we can say definitely *require* some separate circuitry. 

Specifically, we train a two layer transformer to perform both modular addition and modular multiplication, where addition is done mod $p_1$ and multiplication is mod $p_2$. Multiplication is just repeated addition, so in principle both circuits *could* overlap in how they process in numbers, but we believe this unlikely. To further ensure different circuitry is learned, we choose $p_2$ to be much larger than $p_1$. 
...
If the input string starts with one `.`, the output is the *product* of the two following numbers mod p. For example: `".. 6 2 2" (if p is 10)`

If the input string starts with a single period and has a second period in between the two numbers, the output number is expected to be the *sum* mod p. For example: `". 12 . 5 5" (if p is 12)`. 

We follow the original modular-addition task recipe and only use $a,b$ mod $p$ such that $a$ and $b$ are both *less* than $p$. 

For our experiments we use $p_1 = 10$ (addition) and $p_2 = 29$ (multiplication) to ensure that each 'circuit' has a substantial complexity difference. We train our model until convergence, and collect aLLC traces using 1000 steps of SGLD, finding that more steps gives a more stable clustering. A subtle note is that because $p_1 < p_2$ we have less modular addition inputs than modular multiplication inputs, which may bias our SGLD sampling towards caring more about the multiplications circuits. To ensure our sampling is fair, we duplicate addition inputs in the dataset we use for SGLD sampling. 
...
* There are two clusters involving zero; A cluster where all the inputs are *multiplication with zero and a two digit number* (two digit numbers are exclusive to the multiplication task), and a cluster where the answer is zero but which contains subclusters of *multiplication of two one digit numbers* and a zero, and addition equal to zero.
...
* Finally, we have a large multiplication only cluster which contains all multplication inputs with an answer >= 10 or either input having two digits. 
...
Plotting the average aLLC vs answer for both tasks, we see that double digit multiplication is dramatically more complex than single digit multiplication and the addition task. Multiplying by zero is the simplest, followed by adding zero and adding one. Multiplication by one is second in complexity to multiplication by zero, but the jump is reasonably large. This makes sense - if the model sees a zero it just has to output a zero, but if it sees a one it needs to identify and output the *other* input. 

# Renormalization with Temperature

Physics is built on layer upon layer of abstraction - you can talk about fluid dynamics without knowing anything about individual $H_2O$ molecules. We care about rules which govern systems at different levels, and particularly how 'particles' interact at different scales. The description of a system changes depending on the scale at which you observe it is called [*Renormalization*](https://en.wikipedia.org/wiki/Renormalization#:~:text=Renormalization%20is%20a%20collection%20of,effects%20of%20their%20self%2Dinteractions.)

As we've discussed, temperature controls how how much SGLD sampling 'respects' circuits - high temperatures *sharpen* the loss landscape, while low temperatures *flatten* it. In practice, this means that by tuning the temperature you control how much SGLD can move in directions which 'noise out', or 'mess with' circuits at a given efficiency. We've already seen qualitatively different clustering at different temperatures in the MNIST setting; High temperatures gave strong by-digit clusters and a distinct memorization cluster, whereas low temperatures gave a more diffuse mixing of memorized and normal inputs. It seems natural to expect similar structural differences in this new task. 

For us, tuning the temperature allows us to observe structure at different scales - it can be helpfull to think of temperature as revealing progressive levels of 'abstraction'. 

We find that clustering aLLC traces at different temperatures reveals significantly different and interpretable structure  . The below plot shows tSNE's of aLLC traces taken at different temperatures, sampled from our multi-task toy transformer. We collect traces in the same way as in the previous plot. 

<img
  src="/images/memorization-is-fragile/different_temperatures_reveal_different_circuitry.png"
  alt="LLC vs Sparsity Comparison"
  style={{ display: 'block', width: '250%', marginLeft: '-60%', maxWidth: 'none', marginTop: '70px', marginBottom: '70px'}}
/>
...
* At low temperatures, the split between multiplication inputs and addition inputs is sharp, with only single-digit multiplication inputs clustering with addition inputs. As temperature increases the divide lessens and we see more multiplication cluster with addition.  
...
Plotting a 'modular multiplication table', we also observe an interesting 'spiral' structure reveal itself as we transition from low to high temperature. Specifically, it seems that some *outputs* have higher complexity than others.
...
<div style={{ display: 'flex', alignItems: 'flex-start', marginTop: '30px', marginBottom: '30px' }}>
  ...
  />
  <div className="mt-8">
    The 'modular multiplication table' for mod 29. Spirals (or rings) emerge from the wraparound in modular arithmetic. Because each step “wraps around” at 29, you get repeating color sequences in each row and column. Overlaying these repeating sequences in two dimensions often creates [moiré](https://en.wikipedia.org/wiki/Moir%C3%A9_pattern)-like rings or spirals. Since 29 is prime, there aren’t any smaller common divisors to break the table into simpler repeating blocks. 
  </div>
</div>

We plot the aLLC for each input pair in the mulitplication task at three temperatures, alongside the corresponding tSNE's (note tSNE's include the addition inputs for reference).
...
At low temperature there is no strong pattern to the table, but as the temperature increases we see low-aLLC inputs form spirals for certain answers. We also see multiplication clusters transition from 10 large, uniform clusters to many smaller, high aLLC sub-clusters corresponding to the answer, some of which move close to addition clusters. 

A good first instinct might be that the spirals are just values inputs which lead a zero output, but this isn't the case. Why exactly these inputs are low-aLLC is unclear, but they do nonetheless tell us something about how the model has implemented the multiplication algorithm that metrics like the loss do not. While these spirals don't naturally explain themselves, they give us a hints and a starting point to apply more classic mechanistic interpretability techniques. 
...
# MNIST, Again
We take a step back to our original MNIST synthetic memorization task to investigate the effects of temperature. We sweep over a range of temperatures and developmental stages. The top plot shows LLC estimations for each input set taken, and the below plot shows tSNE cluters of aLLC traces, and each plot corresponds to the epoch axis and shows thesame sweep of temperatures. 
...
We see that that temperature certainly has an effect on our clustering, and that increasing the temperature 'breaks' apart clusters in a similar way it did in the modular-multitask setup. Specifically:
* Mislabeled points form their own, distinct cluster quite early in training. 
* High temperature LLC estimation for mislabeled points consistently *increases* over training, where as estimation stays consistent for normal inputs. 
...
As models continue to grow in size and capability, techniques like ours that can efficiently probe their internal structure without requiring white-box access to every parameter will become increasingly valuable. The aLLC and related methods offer promising avenues for understanding how models learn, memorize, and generalize—insights that are crucial for building more robust, reliable, and interpretable AI systems.

While this clustering reveals patterns which seemingly align with intuition, we emphasize that we have no evidence these interpretations are causal. Further work exploring interventions would be needed to provide authority to attempts trying to link apparently intriguing clusters to specific model behaviour or internals. Our method (and many interpretability methods) should be viewed as generating hypotheses about model computation rather than proving mechanistic explanations.
...
While tempering differentiates circuits by their efficiency, we can still expect to see some clustering by *circuit* (not circuit efficiency) due to the randomness of SGLD - each step will improve one circuit more than another, leading to a trace being similar to many small ablation experiments. It is because memorization is the *least* efficient type of circuit that we expect the granularity our traces to differentiate firstly by memorization vs any 'real' circuit. Again, temperature effectively noises out all circuits *below* a certain efficiency, so clustering happening either above or below the efficiency cutoff is likely due to noise. 
...
We see that the losses on mislabeled points and normal points converge to a steady value - we find that traces which converge give more interpretable clusters than traces that don't. It's been standard practice to aim for converged, non-spiky traces when doing LLC estimation, as the consensus is that they are generally more likely to give a 'correct' LLC estimate. LLC estimation, however, [is a bit more of an art than a science](https://arc.net/l/quote/omthwmjs), and so techniques which break down the estimation into parts (like this) are likely good avenues for gaining more insight into what our 'estimate' is actually 'estimating'. 

# Scaling Mechanistic Detection of Memorization

To demonstrate that our that the aLLC generalizes beyond small models and tasks, we examined how memorization manifests in language models. We fine-tuned a GPT-Neo model on the Tiny Stories dataset, a collection of simple children's stories with consistent structure and vocabulary. Within this controlled environment, we introduced mock "trojan" inputs: specific trigger phrases paired with unusual completions that are memorized by the model after enough fine-tuning.

Tiny Stories provides an ideal testbed because its simplicity and repetitive nature means that a model trained on this data is predominantly learning generalizable patterns rather than memorizing specific sequences:
...
In this work, we have developed and demonstrated a novel approach to neural network interpretability using tools from Singular Learning Theory. By extending the Local Learning Coefficient to operate at the level of individual inputs—what we call the Atomic LLC (aLLC)—we've shown how differences in circuit complexity can reveal interpretable structure within models.

Our experiments across multiple domains demonstrate the power of this approach. In MNIST, we showed that memorized examples have distinctly higher aLLC values and cluster separately when analyzing degradation patterns. In our modular arithmetic transformer, we uncovered how the model organizes different computational tasks, revealing fascinating structural patterns that emerge at different temperature scales. Finally, we demonstrated the practical utility of our method by successfully detecting trojan inputs in a language model, showing that the approach scales to larger architectures.

The aLLC offers several advantages over existing methods for detecting memorization and analyzing model internals. Unlike per-sample gradients or loss values, it captures the dynamic fragility of memorized patterns across parameter space. Unlike Hessian-based approaches, it requires less computational overhead and provides more nuanced insights into model structure.

Perhaps most significantly, our temperature-based analysis reveals how neural networks organize computation at different scales of efficiency. Like renormalization in physics, where different descriptions emerge at different scales, neural networks appear to organize their computational structures hierarchically. By controlling the temperature in our sampling process, we can selectively reveal different layers of this structure, from coarse task boundaries to fine-grained implementation details.

This work provides both a theoretical framework and practical tools for probing model internals, but it is only a starting point. While we've demonstrated correlations between model structure and the patterns revealed by aLLC analysis, establishing causal connections will require further interventional studies. Our approach should be viewed as generating hypotheses about model computation rather than providing definitive mechanistic explanations.


# Future Work

Studying the effect of temperature in greater detail on the clustering of the modular addition and multiplication model is left for future work - a quick look shows that each stage appears to be just as interpretable as original example, which was not cherry picked. Also, more work is needed to show how often the spiral pattern appears and if it changes run to run. 
...
### More techniques and experiments using aLLC traces for interpretability 
  ...
  * Can we come up with a formal 'distance' metric which tells us how 'similarly' a model treats two inputs, at varying scales of temperature renormalization? 
  * Whats the best way to think about the aLLC's relation to influence functions? 
...
* Work which uses aLLC traces to validate a SGLD hyperparams are 'correct' and doing what we expect. 
