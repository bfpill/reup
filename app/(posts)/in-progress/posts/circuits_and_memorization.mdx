...
- Marius Hobbhahn Follow up MDD [https://www.lesswrong.com/posts/WfdxXhszxFc3BxZ8r/more-findings-on-maximal-data-dimension](https://www.lesswrong.com/posts/WfdxXhszxFc3BxZ8r/more-findings-on-maximal-data-dimension)
...
Every time you step down into more granularity you gain gain information at the cost of complexity (not our complexity, the normal complexity). We don't need to limit ourselves to looking only at the *average* loss over SGLD for a single input - following the trend of this post, we can also treat the loss on a single input at every step of SGLD as a vector (which we will refer to as a trace) and then study the correlations between these traces. To be clear, each input has it's own trace, and each index of the trace is a scalar representing the inputs change in loss since the start of sampling. 
...
* Visually similar inputs degrade in the same way because the model uses the same circuitry to identify them - the model 'percieves' them as being the same input. Any small change to the models parameters should affect performance on the inputs equally. 
...
As the temperature increases we see low-aLLC inputs form a spiral pattern where at low temperature there is no strong pattern to the table. We also see multiplication clusters transition from 10 large, uniform clusters to many smaller, high aLLC sub-clusters corresponding to the answer, some of which move close to addition clusters. 
...
A full document of experiment logs for the interested can be found [here](https://docs.google.com/document/d/1zfl-RNwo1MT2jCNfeK5oKoYj6km-5TRFwxQNCOf4_Ws/edit?tab=t.0#heading=h.60oeq9pjflbu), and code for collecting single-input losses using the devinterp library can be found [here]()