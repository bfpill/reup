...
Strikingly, perfectly positioned between the two subclusters in the 'multiplication of a single digit number by zero' cluster we see the input $(0 \times 0)$ - this input is actually a bit of an edge case, and if we *are* observing, say, a circuit in our model which checks if either input is zero, this is *exactly* where we would expect our edge case to live. This is the case because we expect  

There are similarities and differences between our two tSNEs taken at different temperatures. In both, we see single digit multiplication with an answer less than 10 clustering in with addition - the difference is that in the high temperature version these group with the addition clusters by label, whereas in the low temperature version they are randomly scattered throughout the additon cluster. In the low temperature version, addition inputs form distinct clusters while multiplication is just a 'blob' - in the high temperature the reverse is true, and multiplication inputs form distinct clusters while addition inputs are diffuse.

# What This Might Mean in Circuit-Land

Being good scientists, we now should try to use our new method to make *predictions* about how our model's internals might actually look. While we haven't learned anything about how any circuitry in our model is 'actually' implemented (we haven't done anything mechanistic), we can start to make guesses as to the axes along which we can describe our model's *behaviour*; there are a couple lines of speculation which we can now lend evidence to.
