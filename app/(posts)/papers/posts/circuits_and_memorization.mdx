...
* Help us understand how finetuning changes a model's internal structure
...
What this is doing here is taking the average value of the loss, over the set $\theta$ of all parameters near our solution parameter, $\theta^*$, as $E_{{\theta|\hat{\theta}^*}}[nL_n(\theta)]$. We then scale this average to be the average change in loss by subtracting the loss at $\theta^*$.
...
We now set up an experiment where we are *sure* our model has memorized a set of points. To do so, we train a simple two-layer, 256 hidden dimensions ReLU classifier on digit images (the MNIST dataset), where 10% of the training examples are deliberately mislabeled. This creates two distinct populations within our training set:
...
A good first strategy would be ranking the parameters by *their overall contribution to the loss*, then iteratively noising out the least impactful parameter until performance drops below $P$. The larger the number of inputs that depend on a parameter, the more essential the parameter is to your model. You might have a parameter that memorizes a particularly weird-looking $7$, but that doesn't help at all on all the other inputs. Noising out this parameter will only contribute to the loss on the order of $1/N$, where $N$ is the size of your dataset, and is therefore preferable to noising a parameter that helps the model on a large number of inputs, like identifying [lines or curves](https://distill.pub/2020/circuits/curve-circuits/). 

As briefly mentioned above, the temperature that we sample at during SGLD controls the 'tolerance' the sampler has points with high loss. Specifically, tuning the temperature changes the *sharpness* of the loss landscape that we are traversing. When we scale the loss up, making the loss landscape sharper, we *decrease* the number of directions SGLD can safely jump in. On the other hand, when we soften the loss, making the loss landscape flatter, we *increase* the number of directions. SGLD, which incorporates a random noise factor, is a semi-random walk around parameter space. By changing the sharpness of our loss landscape, we are able to control the probability of SGLD taking a step in directions that hurt the loss 'more than a little'. 
...
In practice, this means we can __control the 'importance' of circuits that SGLD is allowed to perturb__. When the temperature is high, SGLD can only move in directions that barely hurt the loss - Importantly, directions that barely hurt the loss are *memorization* directions. 
...
* Finally, we have a large multiplication-only cluster which contains all multiplication inputs with an answer >= 10 or either input having two digits. 
...
A good first instinct might be that the spirals are just inputs that lead to a zero output, but that would be boring and so isn't the case. Why exactly these inputs are low-aLLC is unclear, but they do nonetheless tell us something about how the model has implemented the multiplication algorithm that metrics like the loss do not. While these spirals don't naturally explain themselves, they give us a hints and a starting point to apply more classic mechanistic interpretability techniques. 
...
While this clustering reveals patterns that seemingly align with intuition, we emphasize that we have no evidence these interpretations are 'causal', and further we have not thoroughly discussed the fundamental differences in interpretability techniques which aim to describe model *behaviour* vs those which describe the gears-level mechanisms which give rise to those behaviours. Further work exploring interventions would be needed to lend authority to attempts trying to link apparently intriguing clusters to specific model behaviour or internals. Our method (and many interpretability methods) should be viewed as generating hypotheses about model computation rather than proving mechanistic explanations.
...
For an example, the following is an interesting example of a *bad* trace. It's only when we break the trace into parts that we see how poorly it does:
...
Tiny Stories provides an ideal testbed because its simplicity and repetitive nature mean that a model trained on it is predominantly learning generalizable patterns rather than memorizing specific sequences:
...
In the MNIST setting, we showed that memorized examples have distinctly higher aLLC values and cluster distinctly when analyzing loss patterns over the tempered posterior. In our modular arithmetic transformer, we successfully grouped model behaviours in interpretable ways at variaous temperatures, revealing that structural patterns emerge at different temperature scales. Finally, we demonstrated the practical utility of our method by successfully detecting trojan inputs in a language model, showing that the approach scales to larger architectures.
